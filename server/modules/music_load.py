        ...
        if "," in card_id:
            ...
            if data_cards[card_id][1] == album and data_cards[card_id][2] == artist:
                ...
                if len(data_cards[card_id]) > 3:
                    data_cards[card_id][3] = update_time  # add date when replaced UUID last time
                else:
        ...
        for track_uuid in track_data:
            if album_path in track_data[track_uuid]["file"]:
                ...
                if "#error" in track_info["artist"]:
                    ...
                    if "error" in track_info:    album_data["error"].append(
                        track_info["error"] + " (" + track_info["file"] + ")")
...
def reloadMusic(data, load_all=True, thread=""):
    ...
    if not load_all:
        ...
        file_logging(
            "reloadMusic: Removed files with errors from list of existing files (" + str(len(media_files_exist)) + ")")
    ...
    if "cards" in data:
        ...
        for album_uuid in data_reload["album_info"]:
            cards_temp, card_id = checkIfCardExists(data_cards=cards_temp,
    ...
    load_logging.info(
        "reloadMusic: finished ... " + mbox.music_dir + " | all=" + str(all) + " | albums=" + str(len(data["albums"])))
    file_logging(
        "reloadMusic: finished ... " + mbox.music_dir + " | all=" + str(all) + " | albums=" + str(len(data["albums"])))
    return data_reload
