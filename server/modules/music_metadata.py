...
#md_logging.setLevel(stage.logging_level)
md_logging.setLevel(logging.DEBUG)
...
def readMetadata(path_to_file):
   ...
   # check file type and read metadata
    ...
    elif tags["artist"] == None:                      tags["artist"] = "Unknown Artist"
    if not "album"  in tags or tags["album"] == "":   tags["album"]  = "Unknown Album"
    elif tags["album"] == None:                       tags["album"]  = "Unknown Album"
    if not "title"  in tags or tags["title"] == "":   tags["title"]  = filename
    elif tags["title"] == None:                       tags["title"]  = filename
...
def readMutagen(filename,ftype="mp4"):
    ...
    md_logging.debug(" ... Read metadata (Mutagen::"+ftype+")")
...
def readID3(filename ,album_id="", album_nr=""):
    ...
    if tags_available and audiofile.tag.title is not None:
      ...
      try:
        if "track_num" in tags and tags["track_num"] != None and int(tags["track_num"]) > 0:  tags["sort"] += int(tags["track_num"])
      except Exception as e:
        md_logging.debug(" ... Error in 'track_num' ("+str(tags["track_num"])+"): "+str(e))

      try:
        if "disc_num"  in tags and tags["disc_num"] != None  and int(tags["disc_num"]) > 0:   tags["sort"] += int(tags["disc_num"])*1000
      except Exception as e:
